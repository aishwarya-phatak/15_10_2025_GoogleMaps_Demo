 ...
 *
 * As there is only one info window shown at any time, the returned view may be reused between other
 * info windows.
 *
...
typedef NS_ENUM(NSUInteger, GMSMapViewType) {
  ...
  kGMSTypeNormal NS_SWIFT_NAME(normal) = 1,

      /** Satellite maps with no labels. */
      kGMSTypeSatellite NS_SWIFT_NAME(satellite),

          /** Terrain maps. Supports both Light and Dark color schemes. */
          kGMSTypeTerrain NS_SWIFT_NAME(terrain),

              /** Satellite maps with a transparent label overview. */
              kGMSTypeHybrid NS_SWIFT_NAME(hybrid),

                  /** No maps, no labels.  Display of traffic data is not supported. */
                  kGMSTypeNone NS_SWIFT_NAME(none),
 ...
 */
@property(nonatomic, getter=isTransitEnabled) BOOL transitEnabled;
 ...
 * This affects the color scheme of Normal and Terrain map types, and the default background color
 * of these map types.
 */
